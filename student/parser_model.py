...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(torch.Tensor(n_features * self.embed_size , hidden_size))
    ...
    def embedding_lookup(self, w):
        ...
        output=[]
        for each_index in w:
            selected = torch.index_select(self.embeddings.data, 0 , each_index)
            output.append(selected)
            
        return torch.cat(output, dim=0).view(w.shape[0],-1)
    ...
    def forward(self, w):
        ...
        embeding = self.embedding_lookup(w)
       
        
        h = F.relu(torch.torch.addmm(self.embed_to_hidden_bias, embeding, self.embed_to_hidden_weight))
        
        h_dropout = self.dropout(h)
        
        logits = torch.addmm(self.hidden_to_logits_bias, h_dropout, self.hidden_to_logits_weight)
